...
void tiled_matmul_gemm2(float A[M1][K1], float B[K1][N1], float C[M1][N1]) {
    int SIZE = 32;
    int it, jt, kt, i, j, k;
    int lda = K1;
    int ldb = N1;
...
void tiled_matmul_brgemm(float A[M1][K1], float B[K1][N1], float C[M1][N1]) {
    ...
    /*
    libxsmm_smmfunction_reducebatch_addr kernel = libxsmm_smmdispatch_reducebatch_addr
    (SIZE, SIZE, SIZE,
        &ldb, &lda, &ldc, NULL, NULL, NULL, NULL);
        */
